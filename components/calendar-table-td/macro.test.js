...
describe("calendar-table-td", () => {
  it("renders a <td> with the base class", () => {
    const { document } = render("calendar-table-td", { text: "v" });
    const el = document.querySelector("td.calendar-table-td");
    expect(el).toBeTruthy();
    expect(el.textContent.trim()).toBe("v");
  });

  it("escapes params.text", () => {
    const { document } = render("calendar-table-td", { text: "A & B" });
    expect(document.querySelector("td").textContent.trim()).toBe("A & B");
  });

  it("renders params.html raw", () => {
    const { document } = render("calendar-table-td", {
      html: "<a href='#'>link</a>",
    });
  ...
  it("appends params.classes", () => {
    ...
    expect(document.querySelector("td.calendar-table-td.table-data-numeric")).toBeTruthy();
  });

  it("contains no <style> or <script> tags", () => {
    const { html } = render("calendar-table-td", { text: "x" });
