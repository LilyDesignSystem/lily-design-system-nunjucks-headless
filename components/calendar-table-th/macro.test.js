...
describe("calendar-table-th", () => {
  it("renders a <th> with the base class and default scope", () => {
    const { document } = render("calendar-table-th", { text: "Name" });
    const el = document.querySelector("th.calendar-table-th");
  ...
  it("escapes params.text", () => {
    const { document } = render("calendar-table-th", { text: "A & B" });
    expect(document.querySelector("th").textContent.trim()).toBe("A & B");
  });

  it("renders params.html raw", () => {
    const { document } = render("calendar-table-th", {
      html: "<abbr>BP</abbr>",
    });
  ...
  it("appends params.classes", () => {
    ...
    expect(document.querySelector("th.calendar-table-th.table-col-numeric")).toBeTruthy();
  });

  it("contains no <style> or <script> tags", () => {
    const { html } = render("calendar-table-th", { text: "x" });
