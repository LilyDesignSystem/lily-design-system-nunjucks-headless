...
describe("calendar-table", () => {
  it("renders a <table> with the base class", () => {
    const { document } = render("calendar-table", {});
    expect(document.querySelector("table.calendar-table")).toBeTruthy();
  });

  it("renders a <caption> when params.caption is provided", () => {
    const { document } = render("calendar-table", { caption: "Appointments" });
    const c = document.querySelector("table caption.calendar-table-caption");
    expect(c).toBeTruthy();
  ...
  it("uses aria-label only when no caption is provided", () => {
    const { document } = render("calendar-table", { label: "Appointments" });
  ...
  it("appends params.classes", () => {
    const { document } = render("calendar-table", { classes: "striped" });
    expect(document.querySelector("table.calendar-table.striped")).toBeTruthy();
  });
  ...
  it("contains no <style> or <script> tags", () => {
    const { html } = render("calendar-table", {});
